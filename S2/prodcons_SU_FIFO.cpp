...
// Compilar con g++ -std=c++11 -pthread -o prodconsFIFO prodcons_SU_FIFO.cpp HoareMonitor.cpp
...
private:
  int buffer[tam_vec], // buffer donde almacenamos los valores producidos
    ...
    primera_ocupada; // índice en el vector de la primera celda ocupada
...
void MProdConsSU::escribir(int v) {
  if (contador == tam_vec) // Si no hay espacio para producir, espera
    cola_prod.wait();

  assert(primera_libre < tam_vec);
  contador++; // Actualiza el contador
  buffer[primera_libre] = v; // Escribe en buffer el valor 
  primera_libre = (primera_libre + 1) % tam_vec; // Actualiza el índice
  cola_cons.signal(); 
}	
...
int MProdConsSU::leer() {
  if(contador == 0) // Espera a que se produzcan valores
    cola_cons.wait();
    
  // Operación de lectura, actualizando estado del monitor
  int res = buffer[primera_ocupada];
