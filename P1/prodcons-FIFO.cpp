...
Semaphore sc1 = 1; // semáforo para la sección cŕítica del productor
Semaphore sc2 = 1; // semáforo para la sección crítica del consumidor
...
void funcion_hebra_productora(int n_prod) {
  ...
  for(unsigned i = 0; i < iteraciones; i++) {
    ...
    // Esperamos a que haya huecos libres en el vector
    sem_wait(sem_productor);

    // Sección crítica, debemos garantizar que primera_libre tenga el valor adecuado
    sem_wait(sc1);
    buffer_fifo[primera_libre] = dato; // Escribimos el dato
    primera_libre = (primera_libre + 1) % tam_vec; // Actualizamos el índice
    sem_signal(sc1);

    // Mandamos una señal al consumidor porque tiene otro dato disponible
...
void funcion_hebra_consumidora(int n_cons) {
  ...
  for(unsigned i = 0; i < iteraciones; i++) {
    int dato ;
    // Esperamos a tener datos disponibles
    ...
    sem_wait(sc2);
    dato = buffer_fifo[primera_ocupada]; // Tomamos dato
    primera_ocupada = (primera_ocupada + 1) % tam_vec; // Actualizamos el índice del consumidor
    sem_signal(sc2);
    
    sem_signal(sem_productor); // Hay un hueco libre en el buffer
